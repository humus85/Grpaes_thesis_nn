...
class FullyConnectedModel():
    def __init__(self, limit, label_to_predict = [1], lr = 0.0002, act_func = 0, n_h_layers = 0, n_hidden = 30, n_enc = 20, equal_weights = True):
    ...
    def create_model(self):
        ...
        self.net = Net(n_feature=n_features, n_hidden=self.n_hidden, n_h_layers=self.n_h_layers, n_enc=self.n_enc, act_func=self.act_func, n_output=n_labels)  # define the network
        self.net = self.net.float()
        self.net.apply(init_weights)
        if self.equal_weights:
            self.loss_weights = [1]*n_labels

    def fit(self, X_train_t, y_train_t, X_test_t, y_test_t, eval = True, stop=False, plot = False):
        ...
        self.loss_weights = [self.a1, self.a2, self.a3,self.a4,self.a5,self.a6,self.a7]
        last_10000_loss = float('inf')
        while not stop and self.n_epochs <= 30000:
            self.n_epochs += 1
            self.net.train()
            ...
            if self.n_epochs%2==0:
                ...
                loss = 100 * loss_func(prediction[:, self.label_to_predict ], self.y_train[:, self.label_to_predict ].float())
            ...
            if eval and self.n_epochs > 1:
                ...
                with torch.no_grad():
                    ...
                    eval_loss = loss_func(pred_test[:, self.label_to_predict ], self.y_test[:, self.label_to_predict ].float())  # must be (1. nn output, 2. target)
